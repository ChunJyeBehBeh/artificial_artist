...
def x_end(theta):
    ...
    x = length[1]*cos(r(theta[0]))*sin(r(theta[1])) - (cos(r(theta[0]))*sin(r(theta[1]))*sin(r(theta[2])) - cos(r(theta[0]))*cos(r(theta[1]))*cos(r(theta[2])))*(length[3]*cos(r(theta[3])) - length[4]*sin(r(theta[3]))) - (cos(r(theta[0]))*cos(r(theta[1]))*sin(r(theta[2])) + cos(r(theta[0]))*cos(r(theta[2]))*sin(r(theta[1])))*(length[4]*cos(r(theta[3])) + length[3]*sin(r(theta[3]))) + length[2]*cos(r(theta[0]))*cos(r(theta[1]))*cos(r(theta[2])) - length[2]*cos(r(theta[0]))*sin(r(theta[1]))*sin(r(theta[2]))
...
def z_end(theta):
    ...
    length: List[4] = length1, length2, length3, length4 List of joint length'''
    z = length[0] + length[1]*cos(r(theta[1])) - (cos(r(theta[1]))*sin(r(theta[2])) + cos(r(theta[2]))*sin(r(theta[1])))*(length[3]*cos(r(theta[3])) - length[4]*sin(r(theta[3]))) - (cos(r(theta[1]))*cos(r(theta[2])) - sin(r(theta[1]))*sin(r(theta[2])))*(length[4]*cos(r(theta[3])) + length[3]*sin(r(theta[3]))) - length[2]*cos(r(theta[1]))*sin(r(theta[2])) - length[2]*cos(r(theta[2]))*sin(r(theta[1]))
    return z
